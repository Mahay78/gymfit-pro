...
export function Header({ isOnline, workoutActive, workoutPhase, activeWorkoutTime }: Props) {
  return (
    <header className="sticky top-0 bg-slate-900/95 backdrop-blur-md border-b border-slate-800 z-40 px-4 py-3 shadow-md">
      <div className="max-w-5xl mx-auto flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
          <div className="bg-emerald-500 text-slate-950 p-2 sm:p-2.5 rounded-xl shadow-lg shadow-emerald-500/20 flex-shrink-0">
          ...
          <div className="min-w-0">
            <h1 className="text-base sm:text-lg font-black tracking-tight bg-gradient-to-r from-emerald-400 to-teal-300 bg-clip-text text-transparent truncate">
              GymFit Pro
            </h1>
        ...
        {workoutActive && workoutPhase !== 'victory' && (
          <div className="flex items-center gap-2 bg-rose-500/10 border border-rose-500/30 px-3 py-1.5 rounded-xl flex-shrink-0">
            <span className="w-2.5 h-2.5 rounded-full bg-rose-500 animate-ping" />
            <span className="text-xs font-bold text-rose-400 font-mono">{formatTime(activeWorkoutTime)}</span>
          </div>
        )}
      </div>
    </header>
